...
bool Settings::SetString(const std::string& key, const std::string& value) {
    if (read_write_) {
        ...
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "Failed to write string ns=%s key=%s: %s", ns_.c_str(), key.c_str(), esp_err_to_name(ret));
            return false;
        }
        dirty_ = true;
        return true;
...
bool Settings::SetInt(const std::string& key, int32_t value) {
    if (read_write_) {
        ...
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "Failed to write int ns=%s key=%s: %s", ns_.c_str(), key.c_str(), esp_err_to_name(ret));
            return false;
        }
        dirty_ = true;
        return true;
...
bool Settings::SetBool(const std::string& key, bool value) {
    if (read_write_) {
        ...
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "Failed to write bool ns=%s key=%s: %s", ns_.c_str(), key.c_str(), esp_err_to_name(ret));
            return false;
        }
        dirty_ = true;
        return true;
...
bool Settings::EraseKey(const std::string& key) {
    if (read_write_) {
        ...
        if (ret != ESP_ERR_NVS_NOT_FOUND) {
            if (ret != ESP_OK) {
                ESP_LOGE(TAG, "Failed to erase key ns=%s key=%s: %s", ns_.c_str(), key.c_str(), esp_err_to_name(ret));
                return false;
            }
            dirty_ = true;
        }
