...
enum class ServoPanCalibrationAction {
    None,
    Start,
    StepLeft,
    StepRight,
...
public:
    ...
    virtual bool HasCameraCapability() { return GetCamera() != nullptr; }
    ...
    virtual void SetNextCaptureProfile(CameraCaptureProfile profile) { (void)profile; }
    virtual CameraViewPose GetCameraViewPose() const { return CameraViewPose::Default; }
    virtual bool SetCameraViewPose(CameraViewPose pose) { (void)pose; return false; }
    virtual bool SetServoPanForTest(float pan_angle, uint32_t duration_ms) {
        (void)pan_angle;
        (void)duration_ms;
        return false;
    }
    ...
    virtual bool AdjustServoPanCalibration(float delta_deg, std::string* result = nullptr) {
        (void)delta_deg;
        if (result != nullptr) {
            *result = "";
        }
    ...
    virtual bool IsServoPanCalibrationActive() const { return false; }
    virtual std::string GetLastCaptureDebugJson() { return "{}"; }
    virtual bool GetLastCaptureJpeg(std::string& jpeg_data, std::string* mime_type = nullptr) {
        (void)jpeg_data;
    ...
    virtual bool CaptureOnce(uint8_t** out_buf, size_t* out_len) {
        (void)out_buf;
        (void)out_len;
        return false;
    }
