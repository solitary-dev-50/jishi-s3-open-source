...
private:
    friend class McpCameraToolsRegistrar;
    friend class McpProtocolController;

    McpServer();
    ~McpServer();

    void ParseCapabilities(const cJSON* capabilities);
    std::string ExplainCapturedPhoto(const uint8_t* jpeg_buf, size_t jpeg_len, const std::string& question) const;
