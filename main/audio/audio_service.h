...
struct AudioQueueDiagnostics {
    size_t send_queue_size = 0;
    size_t send_queue_high_watermark = 0;
    size_t encode_queue_size = 0;
    size_t encode_queue_high_watermark = 0;
    uint32_t send_packets_enqueued = 0;
    uint32_t send_packets_popped = 0;
    uint64_t send_bytes_enqueued = 0;
    uint64_t send_bytes_popped = 0;
};
...
public:
    ...
    size_t GetEncodeQueueSize();
    AudioQueueDiagnostics GetQueueDiagnostics();
...
private:
    ...
    size_t encode_queue_high_watermark_ = 0;
    uint32_t send_packets_enqueued_ = 0;
    uint32_t send_packets_popped_ = 0;
    uint64_t send_bytes_enqueued_ = 0;
    uint64_t send_bytes_popped_ = 0;
    std::atomic<uint8_t> input_level_{0};
    std::atomic<uint8_t> output_level_{0};
    std::atomic<uint32_t> input_level_timestamp_ms_{0};
    std::atomic<uint32_t> output_level_timestamp_ms_{0};
    ...
    void OpusCodecTask();
    void EnsureTaskStorageAllocated();
    bool WaitForTaskExit(TaskHandle_t* task_handle, const char* task_name, uint32_t timeout_ms = 2000);
    void PushTaskToEncodeQueue(AudioTaskType type, std::vector<int16_t>&& pcm);
    void SetDecodeSampleRate(int sample_rate, int frame_duration);
    void CheckAndUpdateAudioPowerState();
    void UpdateInputLevel(const std::vector<int16_t>& data);
    void UpdateOutputLevel(const std::vector<int16_t>& data);
};
