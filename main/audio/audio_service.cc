...
namespace {
constexpr uint32_t kAudioInputStackBytes = 2048 * 3;
constexpr uint32_t kAudioOutputStackBytes = 2048 * 2;
constexpr uint32_t kOpusCodecStackBytes = 2048 * 12;
constexpr uint32_t kAudioLevelStaleMs = 220;

uint8_t ComputeAudioLevel(const std::vector<int16_t>& data) {
    if (data.empty()) {
        return 0;
    }
    ...
    for (int16_t sample : data) {
        peak = std::max<int32_t>(peak, std::abs(static_cast<int32_t>(sample)));
    }

    // Compress the raw PCM peak into a stable UI-friendly 0-255 range.
    const float normalized = std::clamp(static_cast<float>(peak) / 6000.0f, 0.0f, 1.0f);
...
uint8_t AudioService::GetInputLevel() const {
    ...
    const uint32_t last_ms = input_level_timestamp_ms_.load(std::memory_order_relaxed);
    const uint8_t level = input_level_.load(std::memory_order_relaxed);
    if (now_ms - last_ms > kAudioLevelStaleMs) {
        return 0;
    }
    return level;
}

uint8_t AudioService::GetOutputLevel() const {
    const uint32_t now_ms = static_cast<uint32_t>(esp_log_timestamp());
    const uint32_t last_ms = output_level_timestamp_ms_.load(std::memory_order_relaxed);
    const uint8_t level = output_level_.load(std::memory_order_relaxed);
    if (now_ms - last_ms > kAudioLevelStaleMs) {
        return 0;
    }
    return level;
...
void AudioService::Initialize(AudioCodec* codec) {
    ...
    // The AFE library emits frequent empty-ringbuffer warnings during normal
    // listen/speak transitions. Keep only real errors.
...
void AudioService::Start() {
    EnsureTaskStorageAllocated();
    if (audio_input_task_buffer_ == nullptr || audio_output_task_buffer_ == nullptr ||
        opus_codec_task_buffer_ == nullptr || audio_input_task_stack_ == nullptr ||
        audio_output_task_stack_ == nullptr || opus_codec_task_stack_ == nullptr) {
        ESP_LOGE(TAG, "Audio task storage not ready, cannot start audio service");
        return;
    }
    ...
    if (audio_input_task_handle_ == nullptr) {
        audio_input_task_handle_ = xTaskCreateStaticPinnedToCore([](void* arg) {
            ...
            vTaskDelete(NULL);
        }, "audio_input", kAudioInputStackBytes, this, 8, audio_input_task_stack_, audio_input_task_buffer_, 0);
        if (audio_input_task_handle_ == nullptr) {
    ...
    if (audio_output_task_handle_ == nullptr) {
        audio_output_task_handle_ = xTaskCreateStaticPinnedToCore([](void* arg) {
            ...
            vTaskDelete(NULL);
        }, "audio_output", kAudioOutputStackBytes, this, 4, audio_output_task_stack_, audio_output_task_buffer_, tskNO_AFFINITY);
        if (audio_output_task_handle_ == nullptr) {
    ...
    if (opus_codec_task_handle_ == nullptr) {
        opus_codec_task_handle_ = xTaskCreateStaticPinnedToCore([](void* arg) {
            ...
            vTaskDelete(NULL);
        }, "opus_codec", kOpusCodecStackBytes, this, 2, opus_codec_task_stack_, opus_codec_task_buffer_, tskNO_AFFINITY);
        if (opus_codec_task_handle_ == nullptr) {
...
void AudioService::Stop() {
    ...
    WaitForTaskExit(&audio_input_task_handle_, "audio_input");
    WaitForTaskExit(&audio_output_task_handle_, "audio_output");
    WaitForTaskExit(&opus_codec_task_handle_, "opus_codec");
}
                    ...
                    if (task->type == kAudioTaskTypeEncodeToSendQueue) {
                        {
                            ...
                            send_queue_high_watermark_ = std::max(send_queue_high_watermark_, audio_send_queue_.size());
                            ++send_packets_enqueued_;
                            send_bytes_enqueued_ += audio_send_queue_.back()->payload.size();
                        }
...
void AudioService::EnsureTaskStorageAllocated() {
    if (audio_input_task_stack_ == nullptr) {
        audio_input_task_stack_ = AllocateTaskStack("audio_input", kAudioInputStackBytes);
    }
    if (audio_output_task_stack_ == nullptr) {
        audio_output_task_stack_ = AllocateTaskStack("audio_output", kAudioOutputStackBytes);
    }
    if (opus_codec_task_stack_ == nullptr) {
        opus_codec_task_stack_ = AllocateTaskStack("opus_codec", kOpusCodecStackBytes);
    }
    if (audio_input_task_buffer_ == nullptr) {
        audio_input_task_buffer_ = AllocateTaskBuffer("audio_input");
    }
    if (audio_output_task_buffer_ == nullptr) {
        audio_output_task_buffer_ = AllocateTaskBuffer("audio_output");
    }
    if (opus_codec_task_buffer_ == nullptr) {
        opus_codec_task_buffer_ = AllocateTaskBuffer("opus_codec");
    }
}

bool AudioService::WaitForTaskExit(TaskHandle_t* task_handle, const char* task_name, uint32_t timeout_ms) {
    if (task_handle == nullptr || *task_handle == nullptr) {
        return true;
    }

    const TickType_t delay_ticks = pdMS_TO_TICKS(10);
    const uint32_t max_rounds = std::max<uint32_t>(1, timeout_ms / 10);
    ...
    ESP_LOGW(TAG, "Timed out waiting for %s task to exit", task_name);
    return false;
}
...
void AudioService::PushTaskToEncodeQueue(AudioTaskType type, std::vector<int16_t>&& pcm) {
    ...
    encode_queue_high_watermark_ = std::max(encode_queue_high_watermark_, audio_encode_queue_.size());
    audio_queue_cv_.notify_all();
}
...
std::unique_ptr<AudioStreamPacket> AudioService::PopPacketFromSendQueue() {
    ...
    ++send_packets_popped_;
    send_bytes_popped_ += packet->payload.size();
    audio_queue_cv_.notify_all();
    return packet;
}
...
size_t AudioService::GetEncodeQueueSize() {
    std::lock_guard<std::mutex> lock(audio_queue_mutex_);
    return audio_encode_queue_.size();
}

AudioQueueDiagnostics AudioService::GetQueueDiagnostics() {
    std::lock_guard<std::mutex> lock(audio_queue_mutex_);
    AudioQueueDiagnostics diagnostics;
    diagnostics.send_queue_size = audio_send_queue_.size();
    diagnostics.send_queue_high_watermark = send_queue_high_watermark_;
    diagnostics.encode_queue_size = audio_encode_queue_.size();
    diagnostics.encode_queue_high_watermark = encode_queue_high_watermark_;
    diagnostics.send_packets_enqueued = send_packets_enqueued_;
    diagnostics.send_packets_popped = send_packets_popped_;
    diagnostics.send_bytes_enqueued = send_bytes_enqueued_;
    diagnostics.send_bytes_popped = send_bytes_popped_;
    return diagnostics;
}
...
void AudioService::EnsureOutputReady() {
    if (codec_ == nullptr) {
        return;
    }
    if (!codec_->output_enabled()) {
...
void AudioService::UpdateInputLevel(const std::vector<int16_t>& data) {
    const uint8_t target = ComputeAudioLevel(data);
    const uint8_t current = input_level_.load(std::memory_order_relaxed);
    const uint8_t smoothed = static_cast<uint8_t>((static_cast<uint16_t>(current) * 3 + target) / 4);
    input_level_.store(smoothed, std::memory_order_relaxed);
    input_level_timestamp_ms_.store(static_cast<uint32_t>(esp_log_timestamp()), std::memory_order_relaxed);
}

void AudioService::UpdateOutputLevel(const std::vector<int16_t>& data) {
    const uint8_t target = ComputeAudioLevel(data);
    const uint8_t current = output_level_.load(std::memory_order_relaxed);
    const uint8_t smoothed = static_cast<uint8_t>((static_cast<uint16_t>(current) * 3 + target) / 4);
    output_level_.store(smoothed, std::memory_order_relaxed);
    output_level_timestamp_ms_.store(static_cast<uint32_t>(esp_log_timestamp()), std::memory_order_relaxed);
}
