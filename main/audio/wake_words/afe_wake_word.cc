...
namespace {
constexpr uint32_t kAudioDetectionStackBytes = 4096;
...
bool AfeWakeWord::Initialize(AudioCodec* codec, srmodel_list_t* models_list) {
    ...
    audio_detection_task_stack_ = AllocateTaskStack("audio_detection", kAudioDetectionStackBytes);
    audio_detection_task_buffer_ = AllocateTaskBuffer("audio_detection");
    if (audio_detection_task_stack_ == nullptr || audio_detection_task_buffer_ == nullptr) {
        return false;
    }

    audio_detection_task_handle_ = xTaskCreateStatic([](void* arg) {
        auto this_ = (AfeWakeWord*)arg;
        this_->AudioDetectionTask();
        this_->audio_detection_task_handle_ = nullptr;
        vTaskDelete(NULL);
    }, "audio_detection", kAudioDetectionStackBytes, this, 3, audio_detection_task_stack_, audio_detection_task_buffer_);
...
void AfeWakeWord::AudioDetectionTask() {
    ...
    while (!detection_task_shutdown_) {
        auto bits = xEventGroupWaitBits(event_group_, DETECTION_RUNNING_EVENT, pdFALSE, pdTRUE, pdMS_TO_TICKS(100));
        if (detection_task_shutdown_) {
            break;
        }
    ...
    ESP_LOGW(TAG, "Audio detection task stopped");
}
...
void AfeWakeWord::WaitForDetectionTaskExit(uint32_t timeout_ms) {
    if (audio_detection_task_handle_ == nullptr) {
        return;
    }
    ...
    ESP_LOGW(TAG, "Timed out waiting for audio_detection task to exit");
}
